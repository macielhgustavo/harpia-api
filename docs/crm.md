...
- `GET /crm/opportunities/:id/unit-matches?page=1&pageSize=20` — `{ data, reason, pagination: { page, pageSize, total, totalPages } }`, máximo 100. Sem perfil: `200`, `data: []`, `reason: "PROFILE_REQUIRED"`; oportunidade de outro tenant: `404`. Exige `CRM_READ`, sem `organizationId` do cliente.
- `PATCH /crm/opportunities/:id` com `{ developmentId, unitId }` seleciona uma unidade recomendada. Para um `unitId` novo, a transação bloqueia a linha da unidade e confirma tenant, status disponível, empreendimento não cancelado, preço em tabela ativa e ausência de reserva/venda ativa antes de gravar; estoque alterado retorna `409`. O empreendimento informado deve corresponder à unidade. Selecionar não altera interesse nem cria/cancela reserva ou proposta. Edição sem mudança de unidade mantém a semântica anterior, inclusive quando o estoque da unidade já vinculada mudou.
...
### Matching determinístico de unidades (CRM-017)

`UnitMatchingService` consulta o interesse atual, sem copiá-lo para a unidade nem usar `Opportunity.unitId` como perfil. Filtros rígidos reduzem candidatos no banco: mesmo tenant, `Unit.status = DISPONIVEL`, empreendimento não cancelado, nenhuma reserva `ATIVA`, nenhuma venda `ATIVA`/`QUITADA`, preço em tabela ativa e, quando preenchidos, `developmentId` e `unitTypeId` exatos. A unidade atualmente escolhida pode aparecer, identificada por `isSelected`, sem bônus de ordenação. Estoque sem preço vigente não é recomendado, pois proposta comercial exige esse preço.

Quartos vêm de `UnitType.bedrooms`; área, em m², de `Unit.landArea` para lote e de `Unit.builtArea` (fallback `UnitType.standardArea`) para as demais categorias, arredondada a duas casas. Preço vem da linha `UnitPrice` de tabela ativa mais recente por `updatedAt DESC, id DESC`, exatamente a precedência usada na proposta; é convertido a centavos para comparação/exibição. O `Float` de `UnitPrice` é legado, não introduzido pelo matcher. Quartos, área e preço são critérios suaves: fora da faixa continua visível com `MISMATCH` e diferença de preço exata; atributo ausente é `NOT_EVALUATED`. Entrada disponível e objetivo são exibidos como `NOT_EVALUATED` porque o estoque não define entrada mínima ou adequação objetiva ao objetivo.

Cada item retorna unidade, empreendimento, tipologia, atributos, preço/tabela, contagem de critérios, dados de ordenação e critérios estruturados (`code`, `kind`, `status`, `actual`, faixas e mensagem). A consulta parametrizada calcula filtros, score, total e página em uma operação, sem N+1; a migration aditiva `20260921010000_crm_unit_matching_index` cobre o prefixo de busca `(organizationId, status, developmentId, unitTypeId)`. A relação `UnitPrice(unitId, priceTableId)` e índices existentes de reserva/venda dão suporte aos demais filtros. Paginação é por offset estável para o mesmo snapshot; mudanças no estoque/preços entre requests podem deslocar páginas, então a UI deduplica por ID e a consulta deve ser refeita para visão atualizada. Não há reserva ou seleção automática: o corretor confirma preço/disponibilidade antes de propor.

### Score de compatibilidade (CRM-018)

O mesmo `GET /unit-matches` acrescenta a cada candidato válido `compatibilityScore: number | null`, `compatibilityLevel`, `evaluatedWeight` e três `scoreFactors` estruturados (`criterion`, `weight`, `criterionScore`, `contribution`, `maximumContribution`, `status`, `explanation`). Não há endpoint ou motor de candidatos separado. Os pesos fixos são preço 50, área 25 e quartos 25: o orçamento é a restrição comercial mais forte, enquanto os atributos físicos têm pesos iguais. Empreendimento/tipologia continuam filtros rígidos, sem bônus; disponibilidade e preço válido continuam requisitos de entrada. Entrada disponível e objetivo, sem parâmetro objetivo no estoque, não entram no cálculo.

Cada critério dentro da faixa vale 100. Fora da faixa, o score de preço é `max(0, round(100 − 1000 × desvio / limite violado))`: um desvio de 10% ou mais vale zero. Área usa `max(0, round(100 − 500 × desvio / limite violado))`, zerando em 20%; ambos usam arredondamento decimal half-up e, se o limite violado for zero, score zero. Quartos perdem 50 pontos por quarto além do limite, até zero. O score final é `round_half_up(Σ(score_do_critério × peso) / Σ(pesos_avaliáveis))`, inteiro 0–100. Critério sem preferência ou sem atributo no catálogo fica `NOT_EVALUATED` e sai do numerador e denominador; com nenhum critério avaliável, o score é `null` e o nível `NOT_EVALUATED`, nunca 100 artificial. Fatores mostram contribuição bruta em pontos sobre o peso original, e `evaluatedWeight` explicita a normalização quando há critérios ausentes. Preço é calculado com `Decimal`/`numeric`, sem aritmética monetária em float JS. Níveis: 90–100 `EXCELENTE`, 75–89 `ALTA`, 50–74 `MODERADA`, 0–49 `BAIXA`.

A ordenação global é `compatibilityScore DESC NULLS LAST`, seguida do desempate já existente: preço dentro da faixa, mais soft matches, menos mismatches, menor desvio monetário, `identifier` em collation `C`, `developmentId`, `unitId`. O SQL calcula a mesma fórmula inteira antes de `LIMIT/OFFSET`; a função pura `scoreUnitCompatibility` monta os fatores e confere a pontuação retornada pela consulta para cada item da página. Portanto páginas não são reordenadas separadamente. O detalhe mostra percentual e nível em texto, mais fatores e explicações num painel expansível; isto mede adequação da unidade às preferências atuais, não lead score, probabilidade de compra nem health. Os pesos não são configuráveis nesta fase. Estoque/perfil/preço mudando entre requests ainda podem deslocar páginas por offset, limite herdado do CRM-017.
...
O card de preferências carrega independentemente do restante do detalhe e tem estados próprios de loading, erro/retry e vazio. O formulário inline mantém todos os campos opcionais, valida faixas e dinheiro antes de salvar e busca tipologias do empreendimento selecionado; trocar o empreendimento limpa a tipologia anterior. Se ainda não há perfil, empreendimento e tipologia da unidade atual podem ser sugeridos apenas na abertura do formulário. Editar ou remover o perfil nunca muda a unidade selecionada, que aparece separadamente no card. Ações de escrita só aparecem para `CRM_WRITE`; leitores podem consultar o perfil. Com perfil, leitores e editores podem acionar a busca de unidades compatíveis; os resultados são paginados, explicáveis e têm loading/erro/retry próprios. Remover preferências invalida resultados; salvar invalida páginas antigas e reconsulta a primeira se o painel estava aberto.

CRM-019 refinou os resultados em cards responsivos: score e nível primeiro, unidade/empreendimento/tipologia, preço, atributos disponíveis, disponibilidade e até três razões suaves reais. `Como foi calculado` preserva fatores e critérios completos; `null` continua não avaliado. O link `Ver unidade` abre a lista existente do empreendimento filtrada pelo identificador, pois não existe rota de ficha individual. Seleção usa o PATCH acima; troca exige confirmação, e `409` mantém a oportunidade intacta e atualiza sugestões. Após sucesso, o detalhe é atualizado e aparecem atalhos para as seções existentes de reserva/proposta, sem execução automática. Resultados mantêm a ordenação global e a paginação da API, deduplicando IDs nas páginas anexadas. Não há filtros ou ordenação locais sobre página parcial. No cliente, `Decimal` monetário é apenas formatado como texto/centavos, sem cálculo de score. A disponibilidade mostrada é o snapshot do matching; o PATCH confirma novamente no momento da escolha.
...
**Invariante de ciclo (CRM-FIX-09).** A visita nasce `AGENDADA`. Enquanto agendada, pode mudar data, duração, responsável, local e observações; `PATCH` sem alteração real devolve a visita sem nova escrita ou evento de auditoria. Apenas `AGENDADA → REALIZADA | NAO_COMPARECEU | CANCELADA` é permitido. Esses três estados são finais: correção ou reabertura exigirá um fluxo explícito futuro, não um `PATCH` arbitrário. `REALIZADA` pode registrar `outcome` estruturado, `result` livre e observações; o outcome continua opcional por compatibilidade. `NAO_COMPARECEU` não aceita outcome, result ou motivo de cancelamento. `CANCELADA` exige motivo não vazio (até 500 caracteres pelo DTO) e não aceita outcome, result ou observações de realização. Campos de agenda não podem ser enviados junto da finalização. O writer usa a linha bloqueada por tenant como autoridade e responde `409` para edição de estado final; combinações inválidas respondem `400`. A interface oferece ações apenas para `AGENDADA` e, em conflito, recarrega a visita preservando o aviso. O resultado persistido na visita segue disponível à timeline paginada, sem alterar seu contrato. Nenhuma migration foi necessária.
