...
Cada item retorna unidade, empreendimento, tipologia, atributos, preço/tabela, contagem de critérios, dados de ordenação e critérios estruturados (`code`, `kind`, `status`, `actual`, faixas e mensagem). A ordem total é: preço dentro da faixa primeiro quando ela existir; mais critérios suaves atendidos; menos incompatibilidades; menor desvio monetário; `identifier` em collation `C`; `developmentId`; `unitId`. Não há percentual nem pesos configuráveis. A consulta parametrizada calcula filtros, ranking, total e página em uma operação, sem N+1; a migration aditiva `20260921010000_crm_unit_matching_index` cobre o prefixo de busca `(organizationId, status, developmentId, unitTypeId)`. A relação `UnitPrice(unitId, priceTableId)` e índices existentes de reserva/venda dão suporte aos demais filtros. Paginação é por offset estável para o mesmo snapshot; mudanças no estoque/preços entre requests podem deslocar páginas, então a UI deduplica por ID e a consulta deve ser refeita para visão atualizada. Não há reserva ou seleção automática: o corretor confirma preço/disponibilidade antes de propor.
